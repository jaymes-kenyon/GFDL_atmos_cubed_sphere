...
CONTAINS
    ...
    REAL, DIMENSION( is:ie, js:je, k_start: k_end, moist_count),    &
         INTENT(IN   ) ::              q               !all 3D moisture fields

    REAL, DIMENSION( is:ie, js:je, k_start: k_end),    &
         INTENT(IN   ) ::              t, w, p, z, rho !3D non-moisture fields
    ...
    !from WRF dealing with calling frequency and adaptive time step; call hailcast every timestep for FV3
    !
...
!    IF (run_param) THEN
        ...
        DO j = js, je
          DO i = is, ie
            ...
            ! ----------------------------------------------
             IF (hailcast_wdur(i,j) .gt. 900) THEN
                dhail1=0;dhail2=0;dhail3=0;dhail4=0;dhail5=0
                CALL hailstone_driver (i,j,  t(i,j,k_end:k_start:-1),      &
                                      z(i,j,k_end:k_start:-1),      &
                                      terr_z(i,j),       &
                                      p(i,j,k_end:k_start:-1),      &
                                      rho(i,j,k_end:k_start:-1),    &
                                      qv(i,j,k_end:k_start:-1),     &
              ...
              !ENDIF
              !IF (dhail2 .gt. hailcast_dhail2(i,j)) THEN
                  hailcast_dhail2(i,j) = dhail2
              !ENDIF
              !IF (dhail3 .gt. hailcast_dhail3(i,j)) THEN
                  hailcast_dhail3(i,j) = dhail3
              !ENDIF
              !IF (dhail4 .gt. hailcast_dhail4(i,j)) THEN
                  hailcast_dhail4(i,j) = dhail4
              !ENDIF
              !IF (dhail5 .gt. hailcast_dhail5(i,j)) THEN
                  hailcast_dhail5(i,j) = dhail5
              !ENDIF
            ENDIF
          ENDDO
        ENDDO
        ...
        DO j = js, je
          DO i = is, ie
            !mean
            hailcast_diam_mean(i,j)=(hailcast_dhail1(i,j)+&
                 hailcast_dhail2(i,j) +hailcast_dhail3(i,j)+&
                 hailcast_dhail4(i,j) +hailcast_dhail5(i,j))/5.
            !max
            !hailcast_diam_max(i,j)=MAX(hailcast_dhail1(i,j),&
            !     hailcast_dhail2(i,j), hailcast_dhail3(i,j),&
...
!   Cap updraft lifetime at 2000 sec.
    ...
    !Testing diagnostics that can be commented out, but please keep
    if (0.eq.1) then
       ...
       if (ii.eq. 847 .and. jj.eq.394) then
          ...
          print*,'jmh i=847,j=394 PA:',ii,jj,PA(1:nz)
          print*,'jmh i=847,j=394 rho1d:',ii,jj,rho1d(1:nz)
          print*,'jmh i=847,j=394 RA:',ii,jj,RA(1:nz)
          print*,'jmh i=847,j=394 qi1d:',ii,jj,qi1d(1:nz)
          print*,'jmh i=847,j=394 qc1d:',ii,jj,qc1d(1:nz)
          print*,'jmh i=847,j=394 qr1d:',ii,jj,qr1d(1:nz)
          print*,'jmh i=847,j=394 qs1d:',ii,jj,qs1d(1:nz)
          print*,'jmh i=847,j=394 qg1d:',ii,jj,qg1d(1:nz)
          print*,'jmh i=847,j=394 VUU:',ii,jj,VUU(1:nz)
       ...
       if (KBAS .gt. 30) then
          ...
          print*,'jmh KBAS>30 PA:',ii,jj,PA(1:nz)
          print*,'jmh KBAS>30 rho1d:',ii,jj,rho1d(1:nz)
          print*,'jmh KBAS>30 RA:',ii,jj,RA(1:nz)
          print*,'jmh KBAS>30 qi1d:',ii,jj,qi1d(1:nz)
          print*,'jmh KBAS>30 qc1d:',ii,jj,qc1d(1:nz)
          print*,'jmh KBAS>30 qr1d:',ii,jj,qr1d(1:nz)
          print*,'jmh KBAS>30 qs1d:',ii,jj,qs1d(1:nz)
          print*,'jmh KBAS>30 qg1d:',ii,jj,qg1d(1:nz)
